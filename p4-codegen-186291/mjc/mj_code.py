    ...
    def visit_VarDecl(self, node: VarDecl):
        ...
        # Variable declared in class scope (fields)
            ...
            if node.init is not None:
                ...
                self.text.append((f"field_{var_type}", field_name, value))
            else:
                self.text.append((f"field_{var_type}", field_name, None))
    ...
    def visit_ID(self, node: ID):
        ...
        if var_name in self.name_map:
            ...
            if node.type and node.type.typename.endswith("[]"):
                ...
                node.gen_loc = reg_name  # já é a referência
            else:
                temp = self.new_temp()
